...
- Introduced the offline dinner lottery with 60 foods in 12 categories, five
  selection modes, saved preferences and reroll Easter eggs.
- Added four original screens decorated with fries, a burger, a drink and a
  crowned snowman, plus bilingual play and firmware installation guides.
- Published the tested 1.1.0-dinner merged firmware. Homepage and drawing were
  confirmed on a device; further physical acceptance checks remain documented.

## Inherited upstream template notes

These pending upstream notes predate this application's first release and are
retained as template history, not as this application's release changes.
